package com.chapter08.homework.first_;

/*编程题，要求如下：
    1.在Frock类中声明私有的静态属性currentNum(int类型)，初始化值为：100000,作为衣服的出厂序列号的起始值
    2.声明公有的静态方法getNextNum，作为生成上衣唯一序列号的方法，每调用一次，将currentNum增加100，并作为返回值
    3.在TestFrock类的main方法，分两次调用getNextNum方法，获得序列号并打印输出。
    4.在Frock类中声明serialNumber(序列号)属性，并提供对应的get方法
    5.在Frock类的构造器中，通过调用getNextNum方法为Frock对象获取唯一序列号，赋值给serialNumber属性
