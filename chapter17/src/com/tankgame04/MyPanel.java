...
public class MyPanel extends JPanel implements KeyListener, Runnable {
    ...
    public MyPanel(int key) {
        ...
        switch (key){
            ...
            case 2://表示继续游戏
                ...
                for (int i = 0; i < nodes.size(); i++) {
                    ...
                    enemyTanks.add(new EnemyTank(node_.getX(),node_.getY()));
                    threads[i] = new Thread(enemyTanks.get(i));
                    threads[i].start();
                }
                break;
        ...
        Recorder.setVector(enemyTanks); //获取敌人坦克的集合
...
//    public static void showString(Graphics g){
//        g.setColor(Color.BLACK);
//        Font font = new Font("宋体",Font.BOLD,50);
//        g.setFont(font);
//        g.drawString("是否要继续游戏",200,150);
//    }
    ...
    public void showInfo(Graphics g){
        ...
        g.setColor(Color.BLACK);
        //因为这里需要的是个字符串，因此需要将得到的int类型转换成字符串类型
        g.drawString(Recorder.getAllEnemyNum() + "" ,1100,100);
    ...
    public void paint(Graphics g) { //每次重新调用的是paint函数
        ...
        for (int i = 0; i < enemyTanks.size(); i++) {
            ...
            //让每个坦克都能够指向所有的坦克
            enemyTank.setVector(enemyTanks);
        ...
        switch (tank.getDirect()) {
            ...
            case 2:
                if ((fire.x > tank.getX() && fire.x < tank.getX() + 40) && (fire.y > tank.getY() && fire.y < tank.getY() + 60)) {
                    ...
                    //因为这里是敌方坦克和我方坦克共用一个攻击函数，为了只记录敌方坦克的击毁数量，这里需要加一个判断语句
